...
from solutorresseg.services import get_droplets, get_droplets3
from solutorresseg.services1 import get_licita
from django.contrib.admin.widgets import  AdminDateWidget, AdminTimeWidget, AdminSplitDateTime
from .utils import contextVencedores, contextLicita
...
def asegurar11_view(request,pk):
    print("asegurar1")
    customer = models.Customer.objects.get(user_id=request.user.id)
    ...
    if request.method=='POST':
        ...
        if question1Form.is_valid():
            monedaid = request.POST.get('moneda')
            moneda = CMODEL.Moneda.objects.get(id=monedaid)
            policyid = request.POST.get('policy')
            policy = CMODEL.Policy.objects.get(id=policyid)
            question = question1Form.save(commit=False)
            question.customer=customer
            question.moneda=monedaid
            question.policy=policyid
            question.save()
            
    return render(request,'customer/ask_question1.html',{'question1Form':question1Form,'customer':customer,'context':context})
...
def asegurar_view(request):
    ...
    questions = get_droplets3(spsc)
    paginator = Paginator(questions, paginate_by)
    page_num= request.GET.get("page") or 1
    page_obj = paginator.get_page(page_num)
    ##return render(request, 'customer/asegurar.html', {'questions': questions})
...
def ask_question1_view(request):
    ...
    if request.method=='POST':
        ...
        if question1Form.is_valid():
            ...
            question.policy=policyid
            question.save()
            ####return redirect('question1-history')
    return render(request,'customer/ask_question1.html',{'question1Form':question1Form,'customer':customer})

def question1_history_view(request):
    customer = models.Customer.objects.get(user_id=request.user.id)
    questions = CMODEL.PolizaRecord.objects.all().filter(customer=customer)
    return render(request,'customer/question1_history.html',{'questions':questions,'customer':customer})

class PartidaDetailView(LoginRequiredMixin, DetailView):
    model = PolizaTemp
    template_name = 'customer/ask_question1.html'
    context_object_name = 'partida'

@login_required
def PartidaConcluida(request, pk):
    return render(request, 'customer/ask_question11.html', contextVencedores(pk))
